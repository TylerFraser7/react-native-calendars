...
export default class AgendaView extends Component {
  ...
  onVisibleMonthsChange(months) {
    this.props.visibleMonthsChanged && this.props.visibleMonthsChanged();

    if (this.props.items && !this.state.firstResevationLoad) {
      clearTimeout(this.scrollTimeout);
      this.scrollTimeout = setTimeout(() => {
        if (this.props.loadItemsForMonth && this._isMounted) {
          return months.map(month=>this.props.loadItemsForMonth(month, true))
        }
      }, 200);
    }
  }
  ...
  renderReservations() {
    return (
      <ReservationsList
        ...
        viewingUnreadPosts={this.props.viewingUnreadPosts}
      />
    );
  }
  ...
  onDayChange(day) {
    ...
    const nonEmptyDay = Object.keys(this.props.items).find(day => day === this.state.selectedDay.toString('yyyy-MM-dd'))

    !nonEmptyDay && this.setState({
